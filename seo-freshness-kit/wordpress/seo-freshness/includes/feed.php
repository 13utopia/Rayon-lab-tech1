<?php
/**
 * Self-contained feed. This plugin folder is the whole product — no other repo files.
 */
if (!defined('ABSPATH')) {
  exit;
}

if (function_exists('sf_pad')) {
  return;
}
...
function sf_build_post(array $cfg, array $slot, $index, $now = null) {
  ...
  $category = $cats[$index % max(count($cats), 1)];
  $location = $locs[$index % max(count($locs), 1)];
  $date = sf_display_date($slot['year'], $slot['month'], $slot['day']);
  $title = strtr($templates[$index % max(count($templates), 1)], [
    '{category}' => $category,
    '{location}' => $location,
    '{date}' => $date,
  ]);
...
function sf_posts(array $cfg, $now = null) {
  $out = [];
  foreach (sf_slots($cfg, $now) as $i => $slot) {
